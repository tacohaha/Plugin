...
public class CreateClan {
    ...
    CreateClan(String name, Player player) {
    ...
    private boolean isPlayerInClan(Player player, String clan) {
        if (yamlConfiguration.get("players." + player.getName()) == null) {
            return false;
        } else return yamlConfiguration.get("players." + player.getName() + ".clan") != null | yamlConfiguration.get("players." + player.getName() + ".clan") == clan;
    }
